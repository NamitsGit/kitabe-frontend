...
from mainapp.helpers import genre_wise, tfidf_recommendations, get_book_dict, get_rated_bookids, combine_ids, embedding_recommendations, get_top_n, popular_among_users, get_book_title
...
movies_matrix = ratings_df.pivot_table(index='book_id',columns='user_id',values='rating').fillna(0)
movies_matrix.head()

movies_matrixT=movies_matrix.T

model_knn = NearestNeighbors(metric='cosine', algorithm='brute', n_neighbors=20, n_jobs=-1)
model_knn.fit(movies_matrixT)
...
def book_recommendations(request):
    ...
    if best_user_ratings:
        ...
        recommended_list=[]
        try:
            distances, indices = model_knn.kneighbors(movies_matrixT[movies_matrixT.index==bookid].values.reshape(1, -1), n_neighbors=11)
            for i in range(0, len(distances.flatten())):
                recommended_list.append(movies_matrixT.index[indices.flatten()[i]])
        except:
            recommended_list=[]
        ...
        best_bookids = combine_ids(tfidf_bookids, embedding_bookids, recommended_list)
